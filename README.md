...
商业软件（Melodyne、Auto-Tune）的做法是**级联而非统一**——先时间拉伸修节奏，再音高偏移修音。两者解耦。如果后续加节奏修正，放在音高修正之前即可，不需要改现有管线。

### 换 WORLD 替代 pyrubberband？

- **成本低**：pyworld 已在 `requirements.txt` 中，改动约 40-60 行，离线数据生成多等几十分钟
- **但真正收益不是扩大量程**：超过 ±300 cents 的真实跑调极少。**WORLD 的价值在于分离 F0 和频谱包络后，可以让共振峰也按比例偏移**，让合成跑调数据更像真实跑调，缩小域差距
